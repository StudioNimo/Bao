...
int cmd_help(bao_ctx_t *ctx, int argc, char **argv) {
  ...
  printf(
      ...
      "  run               [-d dataset.jsonl] [--dry-run] (OpenAI; needs OPENAI_API_KEY; more providers: issue #7)\n"
      "  eval              interactive 1/2/3 scoring (TTY); --json planned (issue #5)\n\n"
      "Config & remote:\n"
      "  config            --list, --get, (set = edit bao.yaml)\n"
      "  remote            -v, add, remove, rename, set-url, get-url, show\n\n"
      "Other: push/pull (stubs; issue #6), clone/merge/fetch/rebase/... (stubs)\n");
  return 0;
}
